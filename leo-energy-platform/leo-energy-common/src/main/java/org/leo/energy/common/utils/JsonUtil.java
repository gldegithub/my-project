...
public class JsonUtil {
    ...
    public static Map<String,Object> toMap(Object pojo) {
       return getInstance().convertValue(pojo,new TypeReference<Map<String, Object>>() {});
    }

    /**
     * 将pojo 转换为相关的Map<String,T> 类型的对象
     * @param pojo
     * @param valueTypeRef
     * @param <T>
     * @return
     */
    public static <T> Map<String,T>  toMap(Object pojo,Class<T> valueTypeRef) {
        Map<String, T> newMap = new LinkedHashMap<>();
        return toMap(newMap, pojo, valueTypeRef);
    }

    public static <T> Map<String,T>  toMap(Map<String,T> oldMap,Object pojo,Class<T> valueTypeRef) {
        if (oldMap != null) {
            toMap(pojo).forEach((key, value) -> {
                if (value != null) {
                    if (ClassUtils.isPrimitiveOrWrapper(value.getClass())) {
                        oldMap.put(key, valueTypeRef.cast(value));
                    } else {
                        toMap(oldMap, value, valueTypeRef);
                    }
                } else {
                    oldMap.put(key, null);
                }
    ...
    public static class JacksonObjectMapper extends ObjectMapper {
        ...
        public JacksonObjectMapper() {
            ...
            //日期格式化
            super.registerModule(new JavaTimeModuleExtend());
            super.findAndRegisterModules();
        }
